...
async handleSaveUntilProceed(maxRetries = 6) {
  ...
  async editCourseFromListingPage() {
   ...
   public async selectnewlycreatedLocation(locationName: string) {
    const dropdownXpath = `//input[@id="location_0-filter-field"]`;
    const searchBoxXpath = `//input[@id="location_0" and @placeholder="Search"]`;
    const filteredItemXpath = `//div[@id="lms-resu-disp"]/ul/li`;

    // Step 1: Click the dropdown
    await this.page.locator(dropdownXpath).click();

    // Step 2: Click and fill the search box
    const searchBox = this.page.locator(searchBoxXpath);
    await searchBox.click();
    await searchBox.fill(locationName);

    // Step 3: Wait for filtered results to appear
    await this.page.waitForSelector(filteredItemXpath, { state: 'visible', timeout: 5000 });

    // Step 4: Click the first filtered result
    await this.page.locator(filteredItemXpath).first().click();

    // Optional: Wait for selection to reflect
